...
function format(url, urlType) {
    ...
    if (url instanceof URL) {
        ...
        parsed = url;
        originalType = url_type_1.URL_TYPE.ABSOLUTE;
    }
    else {
        originalType = (0, url_type_1.determineUrlType)(url);
        if (originalType === url_type_1.URL_TYPE.INVALID) {
            throw new Error('Cannot format an invalid URL.');
        }
        if (originalType === url_type_1.URL_TYPE.PATH_RELATIVE) {
    ...
    switch (urlType) {
        case url_type_1.URL_TYPE.PATH_RELATIVE:
            throw new Error('Cannot format into path-relative URLs.');
        case url_type_1.URL_TYPE.PATH_ABSOLUTE:
            return parsed.href.replace(parsed.origin, '');
        case url_type_1.URL_TYPE.SCHEME_RELATIVE:
            if (originalType === url_type_1.URL_TYPE.PATH_ABSOLUTE) {
                throw new Error('Cannot format a path-absolute URL as a scheme-relative URL.');
            }
            return parsed.href.replace(parsed.protocol, '');
        case url_type_1.URL_TYPE.ABSOLUTE:
            if (originalType !== url_type_1.URL_TYPE.ABSOLUTE) {
