...
const TourKitFrame = ({ config }) => {
    ...
    const handleDismiss = useCallback((source) => {
        return () => {
    ...
    const { styles: popperStyles, attributes: popperAttributes, update: popperUpdate, } = usePopper(referenceElement, popperElement, {
        ...
        modifiers: [
            ...
            useMemo(() => liveResizeModifier(config.options?.effects?.liveResize), [config.options?.effects?.liveResize]),
    ...
    const classes = classnames('tour-kit-frame', isMobile ? 'is-mobile' : 'is-desktop', { 'is-visible': tourReady }, classParser(config.options?.classNames));
    ...
    return (_jsxs(_Fragment, { children: [_jsx(KeyboardNavigation, { onMinimize: handleMinimize, onDismiss: handleDismiss, onNextStepProgression: handleNextStepProgression, onPreviousStepProgression: handlePreviousStepProgression, tourContainerRef: tourContainerRef, isMinimized: isMinimized }), _jsxs("div", { className: classes, ref: tourContainerRef, children: [showOverlay() && _jsx(Overlay, { visible: true }), showSpotlight() && (_jsx(Spotlight, { referenceElement: referenceElement, liveResize: config.options?.effects?.liveResize || {}, ...(config.options?.effects?.spotlight || {}) })), _jsxs("div", { className: "tour-kit-frame__container", ref: setPopperElement, ...stepRepositionProps, children: [showArrowIndicator() && (_jsx("div", { className: "tour-kit-frame__arrow", "data-popper-arrow": true, ...arrowPositionProps })), !isMinimized ? (_jsx(TourKitStep, { config: config, steps: config.steps, currentStepIndex: currentStepIndex, onMinimize: handleMinimize, onDismiss: handleDismiss, onNextStep: handleNextStepProgression, onPreviousStep: handlePreviousStepProgression, onGoToStep: handleGoToStep, setInitialFocusedElement: setInitialFocusedElement })) : (_jsx(TourKitMinimized, { config: config, steps: config.steps, currentStepIndex: currentStepIndex, onMaximize: handleMaximize, onDismiss: handleDismiss }))] })] })] }));
};
