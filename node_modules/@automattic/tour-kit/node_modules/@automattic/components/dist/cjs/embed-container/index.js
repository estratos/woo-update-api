...
const dot_pager_1 = tslib_1.__importDefault(require("../dot-pager"));
const noop = () => { };
const debug = (0, debug_1.default)('calypso:components:embed-container');
...
function processEmbeds(domNode) {
    Object.entries(embedsToLookFor).forEach(([embedSelector, fn]) => {
        ...
        (0, lodash_1.forEach)((0, lodash_1.filter)(nodes, nodeNeedsProcessing), fn);
    });
}
...
function loadAndRun(scriptUrl, callback) {
    ...
    if (!loader) {
        loader = new Promise(function (resolve, reject) {
            (0, load_script_1.loadScript)(scriptUrl, function (err) {
                if (err) {
                    reject(err);
                }
                else {
...
function embedTumblr(domNode) {
    ...
    function removeScript() {
        (0, lodash_1.forEach)(document.querySelectorAll('script[src="https://secure.assets.tumblr.com/post.js"]'), function (el) {
            el.parentNode.removeChild(el);
        });
        tumblrLoader = false;
    }
    setTimeout(function () {
        (0, load_script_1.loadScript)('https://secure.assets.tumblr.com/post.js', removeScript);
    }, 30);
}
...
function embedSlideshow(domNode) {
    ...
    (0, lodash_1.forEach)(warningElements, (el) => {
    ...
    if (window.jQuery && window.jQuery.prototype.cycle) {
        ...
        // Neither exist
        (0, load_script_1.loadjQueryDependentScript)(SLIDESHOW_URLS.CYCLE_JS, () => {
            createSlideshow();
        });
    }
}
function embedCarousel(domNode) {
    ...
    if (carouselItemsWrapper) {
        ...
        if (carouselItems && carouselItems.length) {
            (0, client_1.createRoot)(domNode).render((0, jsx_runtime_1.jsx)(dot_pager_1.default, { children: carouselItems.map((item, index) => {
                    return ((0, jsx_runtime_1.jsx)("div", { className: (0, clsx_1.default)('carousel-slide', item?.className), 
...
function embedTiledGallery(domNode) {
    ...
    if (galleryItems && galleryItems.length) {
        ...
        (0, client_1.createRoot)(domNode).render((0, jsx_runtime_1.jsx)("div", { className: "gallery-container", children: imageItems.map((item) => {
                const itemImage = item.querySelector('img');
                const itemLink = item.querySelector('a');
                const imageElement = ((0, jsx_runtime_1.jsx)("img", { id: itemImage?.id || undefined, className: itemImage?.className || undefined, alt: itemImage?.alt || '', src: itemImage?.src || undefined, srcSet: itemImage?.srcSet || undefined }));
                return ((0, jsx_runtime_1.jsx)("figure", { className: "gallery-item", children: (0, jsx_runtime_1.jsx)("div", { className: "gallery-item-wrapper", children: itemLink?.href ? (0, jsx_runtime_1.jsx)("a", { href: itemLink.href, children: imageElement }) : imageElement }) }));
            }) }));
    }
}
/**
 ...
 */
class EmbedContainer extends react_1.PureComponent {
    componentDidMount() {
        processEmbeds(react_dom_1.default.findDOMNode(this));
    }
    componentDidUpdate() {
        processEmbeds(react_dom_1.default.findDOMNode(this));
    }
    componentWillUnmount() {
        ...
        react_dom_1.default.findDOMNode(this)
