...
export function GMClosureNotice({ displayAt, closesAt, reopensAt, enabled }) {
    const { __ } = useI18n();
    if (!enabled) {
        return null;
    }
    ...
    __('Live chat will be closed from %(closes)s – %(reopens)s for the New Year’s holiday', __i18n_text_domain__), {
        closes: format(DATE_FORMAT_SHORT, closesAtDate),
        reopens: format(isSameMonth ? 'd' : DATE_FORMAT_SHORT, reopensAtDate),
    });
    return (_jsx(Panel, { className: "a8c-components__gm-closure-notice", children: _jsx(PanelBody, { initialOpen: period === 'during', title: heading, children: _jsx(PanelRow, { children: MAIN_MESSAGES[period] }) }) }));
}
