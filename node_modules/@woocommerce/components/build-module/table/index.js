    ...
    render() {
        ...
        return (createElement(Card, { className: classes },
            createElement(CardHeader, null,
                ...
                createElement("div", { className: "woocommerce-table__actions" }, actions),
                showMenu && (createElement(EllipsisMenu, { label: __('Choose which values to display', 'woocommerce'), renderContent: () => (createElement(Fragment, null,
                        createElement(MenuTitle, null, __('Columns:', 'woocommerce')),
                        allHeaders.map(({ key, label, required }) => {
                            if (required) {
                                return null;
                            }
                            return (createElement(MenuItem, { checked: showCols.includes(key), isCheckbox: true, isClickable: true, key: key, onInvoke: this.onColumnToggle(key) }, label));
                        }))) }))),
            createElement(CardBody, { size: null }, isLoading ? (createElement(Fragment, null,
                createElement("span", { className: "screen-reader-text" }, __('Your requested data is loading', 'woocommerce')),
                createElement(TablePlaceholder, { numberOfRows: rowsPerPage, headers: headers, rowHeader: rowHeader, caption: title, query: query }))) : (createElement(Table, { rows: rows, headers: headers, rowHeader: rowHeader, caption: title, query: query, onSort: onSort || onQueryChange('sort'), rowKey: rowKey }))),
            createElement(CardFooter, { justify: "center" }, isLoading ? (createElement(TableSummaryPlaceholder, null)) : (createElement(Fragment, null,
                createElement(Pagination, { key: parseInt(query.paged, 10) || 1, page: parseInt(query.paged, 10) || 1, perPage: rowsPerPage, total: totalRows, onPageChange: this.onPageChange, onPerPageChange: onQueryChange('per_page') }),
