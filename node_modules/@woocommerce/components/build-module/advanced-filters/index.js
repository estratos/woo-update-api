...
import { getActiveFiltersFromQuery, getDefaultOptionValue, getNewPath, getQueryFromActiveFilters, getHistory, } from '@woocommerce/navigation';
/**
...
class AdvancedFilters extends Component {
    ...
    getTitle() {
        ...
        return interpolateComponents({
            ...
            components: {
                select: (createElement(SelectControl, { className: "woocommerce-filters-advanced__title-select", options: matches, value: match, onChange: this.onMatchChange, "aria-label": __('Choose to apply any or all filters', 'woocommerce') })),
            },
        });
    }
    getAvailableFilterKeys() {
        ...
        const inactiveFilterKeys = difference(Object.keys(config.filters), activeFilterKeys, multipleValueFilterKeys);
    ...
    render() {
        ...
        return (createElement(Card, { className: "woocommerce-filters-advanced", size: "small" },
            ...
            !!activeFilters.length && (createElement(CardBody, { size: null },
                createElement("ul", { className: "woocommerce-filters-advanced__list", ref: this.filterListRef }, activeFilters
            ...
            availableFilterKeys.length > 0 && (createElement(CardBody, null,
                createElement("div", { className: "woocommerce-filters-advanced__add-filter" },
                    createElement(Dropdown, { className: "woocommerce-filters-advanced__add-filter-dropdown", position: "bottom center", renderToggle: ({ isOpen, onToggle }) => (createElement(Button, { className: "woocommerce-filters-advanced__add-button", onClick: onToggle, "aria-expanded": isOpen },
                            createElement(AddOutlineIcon, null),
                            __('Add a Filter', 'woocommerce'))), renderContent: ({ onClose }) => (createElement("ul", { className: "woocommerce-filters-advanced__add-dropdown" }, availableFilterKeys.map((key) => (createElement("li", { key: key },
                            createElement(Button, { onClick: partial(this.addFilter, key, onClose) }, config.filters[key]
                                .labels.add)))))) })))),
            createElement(CardFooter, { align: "center" },
                createElement("div", { className: "woocommerce-filters-advanced__controls" },
                    updateDisabled && (createElement(Button, { isPrimary: true, disabled: true }, __('Filter', 'woocommerce'))),
                    !updateDisabled && (createElement(Link, { className: "components-button is-primary is-button", type: "wc-admin", href: updateHref, onClick: this.onFilter }, __('Filter', 'woocommerce'))),
                    activeFilters.length > 0 && (createElement(Link, { type: "wc-admin", href: this.getUpdateHref([]), onClick: this.clearFilters }, __('Clear all filters', 'woocommerce')))))));
    }
}
