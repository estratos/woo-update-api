...
const SummaryNumber = ({ children, delta, href, hrefType, isOpen, label, labelTooltipText, onToggle, prevLabel, prevValue, reverseTrend, selected, value, onLinkClickCallback, }) => {
    ...
    let screenReaderLabel = delta > 0
        ? sprintf(__('Up %f%% from %s', 'woocommerce'), delta, prevLabel)
        : sprintf(__('Down %f%% from %s', 'woocommerce'), Math.abs(delta), prevLabel);
    if (!delta) {
        screenReaderLabel = sprintf(__('No change from %s', 'woocommerce'), prevLabel);
    }
    let Container;
    ...
    return (createElement("li", { className: liClasses },
        createElement(Container, { ...containerProps },
            createElement("div", { className: "woocommerce-summary__item-label" },
                ...
                labelTooltipText && (createElement(Tooltip, { text: labelTooltipText, position: "top center" },
                    createElement("div", { className: "woocommerce-summary__info-tooltip" },
                        createElement(Icon, { width: 20, height: 20, icon: info }))))),
            createElement("div", { className: "woocommerce-summary__item-data" },
                createElement("div", { className: "woocommerce-summary__item-value" },
                    createElement(Text, { variant: "title.small", size: "20", lineHeight: "28px" }, !isNil(value)
                        ? value
                        : __('N/A', 'woocommerce'))),
                createElement(Tooltip, { text: !isNil(prevValue)
                        ? `${prevLabel} ${prevValue}`
                        : __('N/A', 'woocommerce'), position: "top center" },
                    createElement("div", { className: "woocommerce-summary__item-delta", role: "presentation", "aria-label": screenReaderLabel },
                        createElement(Text, { variant: "caption", size: "12", lineHeight: "16px" }, !isNil(delta)
                            ? sprintf(__('%f%%', 'woocommerce'), delta)
                            : __('N/A', 'woocommerce'))))),
            onToggle ? (createElement(ChevronDownIcon, { className: "woocommerce-summary__toggle", size: 24 })) : null,
            children)));
};
