...
import { flattenFilters, updateQueryString, getQueryFromActiveFilters, } from '@woocommerce/navigation';
/**
...
class FilterPicker extends Component {
    ...
    renderButton(filter, onClose, config) {
        if (filter.component) {
            ...
            const selectedTag = persistedFilter.value === filter.value
                ...
                : null;
            return (createElement(Search, { autocompleter: autocompleter, className: "woocommerce-filters-filter__search", type: type, placeholder: labels.placeholder, selected: selectedTag ? [selectedTag] : [], onChange: partial(this.onTagChange, filter, onClose, config), inlineTags: true, staticResults: true }));
        }
        ...
        return (createElement(Button, { className: "woocommerce-filters-filter__button", onClick: onClick }, filter.label));
    }
    ...
    render() {
        ...
        const parentFilter = nav.length
            ...
            : false;
        const selectedFilter = this.getFilter();
        return (createElement("div", { className: "woocommerce-filters-filter" },
            config.label && (createElement("span", { className: "woocommerce-filters-label" },
                config.label,
                ":")),
            createElement(Dropdown, { contentClassName: "woocommerce-filters-filter__content", position: "bottom", expandOnMobile: true, headerTitle: __('filter report to show:', 'woocommerce'), renderToggle: ({ isOpen, onToggle }) => (createElement(DropdownButton, { onClick: onToggle, isOpen: isOpen, labels: this.getButtonLabel(selectedFilter) })), renderContent: ({ onClose }) => (createElement(AnimationSlider, { animationKey: nav, animate: animate, onExited: this.onContentMount }, () => (createElement("ul", { className: "woocommerce-filters-filter__content-list" },
                    parentFilter && (createElement("li", { className: "woocommerce-filters-filter__content-list-item" },
                        createElement(Button, { className: "woocommerce-filters-filter__button", onClick: this.goBack },
                            createElement(Icon, { icon: chevronLeft }),
                            parentFilter.label))),
                    visibleFilters.map((filter) => (createElement("li", { key: filter.value, className: classnames('woocommerce-filters-filter__content-list-item', {
