...
class DatePicker extends Component {
    ...
    render() {
        ...
        return (createElement(Dropdown, { position: "bottom center", focusOnMount: false, renderToggle: ({ isOpen, onToggle }) => (createElement(DateInput, { disabled: disabled, value: text, onChange: this.onInputChange, onBlur: partial(this.handleBlur, isOpen, onToggle), dateFormat: dateFormat, label: __('Choose a date', 'woocommerce'), error: error, describedBy: sprintf(__('Date input describing a selected date in format %s', 'woocommerce'), dateFormat), onFocus: partial(this.handleFocus, isOpen, onToggle), "aria-expanded": isOpen, focusOnMount: false, errorPosition: "top center" })), renderContent: ({ onToggle }) => (createElement(Section, { component: false },
                createElement(H, { className: "woocommerce-calendar__date-picker-title" }, __('select a date', 'woocommerce')),
                createElement("div", { className: "woocommerce-calendar__react-dates is-core-datepicker" },
                    createElement(WpDatePicker, { currentDate: date, onChange: partial(this.onDateChange, onToggle), 
